...
var WsStompServer = function (myconfig) {
    ...
    // 静态服务器
    var server = app.listen(config.port, "10.42.1.247");
    ...
    */
    ...
    stompServer.on('subscribe', function (info) {
        console.log("[topic id]" +info.id);
        var clientId = info.id;
        
        stompServer.send("/api", { 'content-type': 'application/json' }, JSON.stringify({
            headers: "good",
            msg: "Ack"
        }));
